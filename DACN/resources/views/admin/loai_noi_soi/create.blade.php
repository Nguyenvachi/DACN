...
@section('title', 'Thêm Loại Nội soi')

@section('content')
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0">+ Thêm Loại Nội soi</h4>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.loai-noi-soi.store') }}">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="ma" class="form-control" value="{{ old('ma') }}" placeholder="VD: NS_CTC">
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="gia" class="form-control" value="{{ old('gia', 0) }}" min="0" step="1000" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Thời gian ước tính (phút) <span class="text-danger">*</span></label>
                        <input type="number" name="thoi_gian_uoc_tinh" class="form-control" value="{{ old('thoi_gian_uoc_tinh', 15) }}" min="1" required>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="phong_id" class="form-select">
                            ...
                            @foreach($phongs as $p)
                                <option value="{{ $p->id }}" {{ (string) old('phong_id') === (string) $p->id ? 'selected' : '' }}>
                                    {{ $p->ten }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="chuyen_khoa_ids[]" class="form-select" multiple>
                            @foreach($chuyenKhoas as $ck)
                                <option value="{{ $ck->id }}" {{ collect(old('chuyen_khoa_ids', []))->map(fn($v)=> (string)$v)->contains((string)$ck->id) ? 'selected' : '' }}>
                                    {{ $ck->ten }}
                                </option>
                            @endforeach
                        </select>
