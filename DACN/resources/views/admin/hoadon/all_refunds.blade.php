    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($hoanTiens as $hoanTien)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('admin.hoadon.show', $hoanTien->hoaDon) }}" class="text-blue-600 hover:text-blue-800 font-medium">
                                #{{ $hoanTien->hoaDon->ma_hoa_don ?? $hoanTien->hoa_don_id }}
                            </a>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm">
                                <div class="font-medium text-gray-900">{{ $hoanTien->hoaDon->user->name ?? 'N/A' }}</div>
                                <div class="text-gray-500">{{ $hoanTien->hoaDon->user->email ?? '' }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-lg font-semibold text-red-600">
                                {{ number_format($hoanTien->so_tien, 0, ',', '.') }} VNĐ
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-900 max-w-xs">{{ $hoanTien->ly_do }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @elseif ($hoanTien->trang_thai === 'Hoàn thành')
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            @if ($hoanTien->trang_thai === 'Đang xử lý')
                                <div class="flex gap-2">
                                    <form method="POST" action="{{ route('admin.hoadon.refund.approve', $hoanTien) }}" class="inline">
                                        @csrf
                                        <button type="submit" 
                                                onclick="return confirm('Xác nhận phê duyệt hoàn tiền {{ number_format($hoanTien->so_tien, 0, ',', '.') }} VNĐ?')"
                                                class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-xs font-medium">
                                            Phê duyệt
                                        </button>
                                    </form>
                                    <button type="button" 
                                            onclick="showRejectModal({{ $hoanTien->id }})"
