@extends('layouts.staff')

@section('title', 'Xét nghiệm chờ thực hiện')

@section('content')
<div class="container-fluid py-4">
    {{-- Header --}}
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 fw-bold text-dark">
                <i class="fas fa-flask me-2 text-warning"></i>
                Xét nghiệm chờ thực hiện
            </h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('staff.dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active">Xét nghiệm chờ</li>
                </ol>
            </nav>
        </div>
        <div>
            <a href="{{ route('staff.xetnghiem.completed') }}" class="btn btn-outline-success">
                <i class="fas fa-check-circle me-2"></i>Đã hoàn thành
            </a>
        </div>
    </div>

    {{-- Stats --}}
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="card border-0 shadow-sm bg-warning bg-opacity-10">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <p class="text-warning fw-bold mb-1 small">CHỜ THỰC HIỆN</p>
                            <h3 class="mb-0 fw-bold">{{ number_format($stats['pending']) }}</h3>
                        </div>
                        <div class="bg-warning bg-opacity-25 rounded p-3">
                            <i class="fas fa-hourglass-start fa-2x text-warning"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card border-0 shadow-sm bg-info bg-opacity-10">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <p class="text-info fw-bold mb-1 small">ĐANG XỬ LÝ</p>
                            <h3 class="mb-0 fw-bold">{{ number_format($stats['processing']) }}</h3>
                        </div>
                        <div class="bg-info bg-opacity-25 rounded p-3">
                            <i class="fas fa-spinner fa-2x text-info"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card border-0 shadow-sm bg-success bg-opacity-10">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <p class="text-success fw-bold mb-1 small">HOÀN THÀNH HÔM NAY</p>
                            <h3 class="mb-0 fw-bold">{{ number_format($stats['completed_today']) }}</h3>
                        </div>
                        <div class="bg-success bg-opacity-25 rounded p-3">
                            <i class="fas fa-check-circle fa-2x text-success"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    {{-- Table --}}
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    <thead class="table-light">
                        <tr>
                            <th width="80">Mã XN</th>
                            <th>Ngày chỉ định</th>
                            <th>Bệnh nhân</th>
                            <th>Bác sĩ</th>
                            <th>Loại xét nghiệm</th>
                            <th width="120">Trạng thái</th>
                            <th width="150" class="text-center">Hành động</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($xetNghiems as $xn)
                        <tr>
                            <td>
                                <strong class="text-primary">#{{ $xn->id }}</strong>
                            </td>
                            <td>
                                <small>{{ $xn->created_at->format('d/m/Y H:i') }}</small>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="avatar-sm bg-light rounded-circle me-2 d-flex align-items-center justify-content-center">
                                        <i class="fas fa-user text-muted"></i>
                                    </div>
                                    <div>
                                        <div class="fw-medium">{{ $xn->benhAn->user->name ?? 'N/A' }}</div>
                                        <small class="text-muted">{{ $xn->benhAn->user->so_dien_thoai ?? '' }}</small>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <small>{{ $xn->bacSi->user->name ?? 'N/A' }}</small>
                            </td>
                            <td>
                                <span class="badge bg-light text-dark">{{ $xn->loai }}</span>
                            </td>
                            <td>
                                @if($xn->trang_thai === 'pending')
                                    <span class="badge bg-warning">
                                        <i class="fas fa-hourglass-start me-1"></i>Chờ
                                    </span>
                                @else
                                    <span class="badge bg-info">
                                        <i class="fas fa-spinner me-1"></i>Đang xử lý
                                    </span>
                                @endif
                            </td>
                            <td class="text-center">
                                @if($xn->trang_thai === 'pending')
                                    <form action="{{ route('staff.xetnghiem.processing', $xn) }}" method="POST" class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-info" title="Bắt đầu xử lý">
                                            <i class="fas fa-play"></i>
                                        </button>
                                    </form>
                                @endif
                                <a href="{{ route('staff.xetnghiem.upload.form', $xn) }}" class="btn btn-sm btn-primary" title="Upload kết quả">
                                    <i class="fas fa-upload"></i>
                                </a>
                                <a href="{{ route('staff.xetnghiem.show', $xn) }}" class="btn btn-sm btn-outline-secondary" title="Xem chi tiết">
                                    <i class="fas fa-eye"></i>
                                </a>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                <i class="fas fa-check-circle fa-3x text-success mb-3 d-block"></i>
                                <h5 class="text-muted">Không có xét nghiệm chờ thực hiện</h5>
                                <p class="text-muted mb-0">Tất cả xét nghiệm đã được xử lý</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>

            {{-- Pagination --}}
            <div class="mt-3">
                {{ $xetNghiems->links() }}
            </div>
        </div>
    </div>
</div>
@endsection

@push('styles')
<style>
.avatar-sm {
    width: 35px;
    height: 35px;
}
</style>
@endpush
